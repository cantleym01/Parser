    ...
    public partial class ParserMain : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            *****************************************************/
            ...
            errors.Add(101, "(Error 1) Invalid identifier found. Only integers or identifiers beginning with \"a-z\", \"A-Z\", and \"_\" are accepted.");
            errors.Add(102, "(Error 2) Parenthesis must have matching pairs and they cannot contain nothing.");
            errors.Add(103, "(Error 3) The assignment statement must have an expression following the \":=\".");
            errors.Add(104, "(Error 4) Statements cannot have anything after the delimiter \";\" on the same line.");
            errors.Add(105, "(Error 5) READ can only a single Identifier or Identifiers separated with commas within it.");
            errors.Add(106, "(Error 6) WRITE can only have expressions within it (Ex: 3 - (x + 4)).");
            errors.Add(109, "(Error 9) Statements must end with a \";\".");
            errors.Add(110, "(Error 10) Code must have three lines minimum. It must start with \"BEGIN\", end with \"END\", and have at least one statement (It is not case sensitive).");
            errors.Add(111, "(Error 11) Unkown Error.");
            errors.Add(112, "(Error 12) Statements must begin with \"READ\", \"WRITE\", or an Identifier. (It is not case sensitive).");
        ...
        private void testButton_Click(object sender, EventArgs e)
        {
            ...
            if (codeTextBox.Text.Length > 0)
            {
                ...
                try
                {
                    ...
                    {
                        throw (new ParserMain.ParserException(1, 110));
                    }
                ...
                catch (ParserException ex)
                {
                    ...
                    messageTextBox.Text = "ERROR at line " + ex.errorLine + ": " + errors.GetByIndex(errors.IndexOfKey(ex.errorCode));
                ...
                messageTextBox.Text = "SUCCESS! Code has ran smoothly.";
                parenthesisChecker.Clear();
                tokens.Clear();
            }
        }
        ...
        {
            //give the text box a scroll bar. text boxes love scroll bars.
            this.codeTextBox.ScrollBars = ScrollBars.Vertical;
        }
        ...
        private void ScrubCode()
        {
            ...
            String[] textArray = codeTextBox.Text.Split(new string[] { "\r\n", "\n" }, StringSplitOptions.None);
        ...
        private void ParseCode()
        {
            ...
            while (currentToken.type != "tail")
            {
                ...
                {
                    throw (new ParserMain.ParserException(tokens[i].codeLine, 112));
                }
                if (tokenArray[j - 1].type != "delimiter")
                {
                    throw (new ParserMain.ParserException(tokens[i].codeLine, 109));
                }
                ...
                {
                    throw (new ParserMain.ParserException(tokens[i].codeLine, 102));
                }
                    ...
                    if (tokenArray[i].value == "read")
                    {
                        ...
                        if (tokenArray[i].type == "leftP")
                        {
                            ...
                            if (tokenArray[i].type != "delimiter" || tokenArray[i - 1].type != "rightP" || (tokenArray[i].type == "delimiter" && tokenArray[i].codeLine == tokenArray[i + 1].codeLine))
                            {
                                ...
                                {
                                    throw (new ParserMain.ParserException(tokens[i].codeLine, 104));
                                }
                                else
                                {
                                    throw (new ParserMain.ParserException(tokens[i].codeLine, 105));
                                }
                            }
                        }
                    ...
                    else //value == write
                    {
                        ...
                        if (tokenArray[i].type == "leftP")
                        {
                            ...
                            if (tokenArray[i].type != "delimiter" || tokenArray[i - 1].type != "rightP" || (tokenArray[i].type == "delimiter" && tokenArray[i].codeLine == tokenArray[i + 1].codeLine))
                            {
                                ...
                                {
                                    throw (new ParserMain.ParserException(tokens[i].codeLine, 106));
                                }
                            }
                        }
                ...
                else if (tokenArray[i].type == "identifier")
                {
                    ...
                    if (tokenArray[i].type == "equals")
                    {
                        ...
                        if (tokenArray[i].type != "delimiter" || (tokenArray[i].type == "delimiter" && tokenArray[i].codeLine == tokenArray[i + 1].codeLine))
                        {
                            ...
                            {
                                throw (new ParserMain.ParserException(tokens[i].codeLine, 103));
                            }
                        }
                    }
                    else //throw error
